import { isPlainObject as adIsPlainObject } from "@hugoalh/advanced-determine";
/**
 * @private
	...
	for (let item of items) {
		for (let itemKey in item) {
			if (Object.prototype.hasOwnProperty.call(item, itemKey)) {
				...
				} else if (item[itemKey] instanceof Map) {
					if (result[itemKey] instanceof Map) {
						for (let [key, value] of item[itemKey].entries()) {
							result[itemKey].set(key, value);
						};
					} else {
						result[itemKey] = item[itemKey];
					};
				} else if (adIsPlainObject(item[itemKey])) {
					result[itemKey] = adIsPlainObject(result[itemKey]) ? concatenatePlainObject(result[itemKey], item[itemKey]) : item[itemKey];
				} else if (item[itemKey] instanceof Set) {
					if (result[itemKey] instanceof Set) {
						for (let value of item[itemKey].values()) {
							result[itemKey].add(value);
						};
					} else {
						result[itemKey] = item[itemKey];
					};
